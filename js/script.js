  ...
  aboutMeSection.innerHTML = mainInformation
    .map(
      ...
      <li class="project-item">
        <a href="${item.github}" class="project-item-link" target="_blank">
          <span class="project-role">${item.title}</span>
          <span class="project-link">${item.preview}</span>
        </a>
      </li>`
            )
            .join('')}
...
function updateWorkExperience(language) {
  ...
  workExperienceSection.innerHTML = `
    <h3 class="main-content-title">${sectionTitle}</h3>
    ${positions
      .map(
        position => `
        <div class="work-experience-place">
          <h4 class="work-experience-position">
            ${position.title}
            <span class="work-experience-company">${position.company}</span>
          </h4>
